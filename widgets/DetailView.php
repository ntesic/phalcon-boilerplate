...
namespace ntesic\boilerplate\Widgets;
...
class DetailView extends Widget
{
    ...
    protected function renderAttribute($attribute, $index)
    {
        if (is_string($this->template)) {
            $captionOptions = Tag::renderTagAttributes(ArrayHelper::getValue($attribute, 'captionOptions', []));
            $contentOptions = Tag::renderTagAttributes(ArrayHelper::getValue($attribute, 'contentOptions', []));
    ...
    protected function normalizeAttributes()
    {
        if ($this->attributes === null) {
            if ($this->model instanceof Model) {
                $this->attributes = $this->model->columnMap();
            } elseif (is_object($this->model)) {
                $this->attributes = array_keys($this->model->toArray());
            } elseif (is_array($this->model)) {
                $this->attributes = array_keys($this->model);
            } else {
                throw new Exception('The "model" property must be either an array or an object.');
            }
...
//                    $attribute['label'] = $this->model instanceof Model ? $this->model->getAttributeLabel($attributeName) : Text::camel2words($attributeName, true);
                    $attribute['label'] = Text::camel2words($attributeName, true);
                }
